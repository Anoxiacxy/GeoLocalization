...
class MappingNetwork(nn.Module):
    def __init__(self,
                 ...
                 w_avg_beta: Optional[float] = 0.995,
...
class SineLayer(nn.Module):
    def __init__(self, in_features, out_features, style_size, bias=False,
                 is_first=False, omega_0: float = 30, weight_modulation=True, **kwargs):
        super().__init__()
...
class GeneratorViT(nn.Module):
    ...
    def forward(self, z):
        ...
        model_output = model_output_2.reshape([-1, self.image_size, self.image_size, self.out_features])

        return model_output

    def forward_token(self, token):
        z = torch.randn(token.shape[0], self.latent_dim, device=token.device)
        w = self.mlp(z)
        pos = repeat(torch.sin(self.pos_emb), 'n e -> b n e', b=z.shape[0])
        hidden = pos + token
        hidden = self.transformer_encoder(hidden, w)
        ...
        model_output_1 = result.view(
            [-1, self.num_patches_x, self.num_patches_x, self.out_patch_size, self.out_patch_size, self.out_features])
        model_output_2 = model_output_1.permute([0, 5, 1, 3, 2, 4])
        model_output = model_output_2.reshape([-1, self.out_features, self.image_size, self.image_size])
