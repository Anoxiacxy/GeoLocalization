...
class PatchDetache(nn.Module):
    ...
    def forward(self, x):
        x = self.norm(x)
        if self.unflatten:
            x = x.transpose(1, 2).unflatten(2, self.grid_size)  # BNC -> BCHW
        x = self.proj(x)
        B, C, H, W = x.shape
...
class vit_decoder(nn.Module):
    def __init__(self, img_size=224, patch_size=16, out_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=nn.LayerNorm, global_pool=None,
                 block_layers=Block,
                 patch_layer=PatchDetache, act_layer=nn.GELU,
...
class vit_encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.dropout_rate:
            x = fc.dropout(x, p=float(self.dropout_rate), training=self.training)
        x = self.head(x)
...
@register_model
def deit_micro_patch16_LS(pretrained=False, img_size=224, pretrained_21k=False, **kwargs):
    model = vit_encoder(
        img_size=img_size, patch_size=16, embed_dim=128, depth=6, num_heads=2, mlp_ratio=4, qkv_bias=True,
